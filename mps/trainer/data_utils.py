...
def create_mnist_dataloader(
    ...
    class FilteredDataset(torch.utils.data.Dataset):
        def __init__(self, dataset, allowed_digits=[0, 1]):
            ...
            self.indices = [
                i for i, (_, label) in enumerate(dataset) if label in allowed_digits
            ]
            self.dataset = dataset

            # Mapping original labels to [0,1,...]
            self.label_to_index = {
                digit: idx for idx, digit in enumerate(allowed_digits)
            }
...
class SyntheticDatasetV2(torch.utils.data.Dataset):
    def __init__(self, n: int, num_samples: int = 10000, seed: int | None = None):
        """
        Create a synthetic dataset where:
        - Label 0 corresponds to states with even parity (even number of 1s)
        - Label 1 corresponds to states with odd parity (odd number of 1s)
    ...
    def __getitem__(self, index):
        if self.seed is not None:
            np.random.seed(self.seed + index)  # Ensure reproducibility per sample
        else:
            np.random.seed(index)

        # Generate a random binary string and compute its parity
        state = np.random.randint(0, 2, size=self.n, dtype=np.int64)
        label = np.sum(state) % 2  # 0 for even parity, 1 for odd parity
        
        # Convert to tensor and create embedded representation
        x = torch.tensor(state, dtype=torch.float64)
        x_embedded = torch.stack([x, 1 - x], dim=-1)
        x_embedded = x_embedded / (x_embedded.sum(dim=-1, keepdim=True).clamp(min=1e-8))
        
        return x_embedded, torch.tensor(label, dtype=torch.int64)


class SyntheticDatasetV3(torch.utils.data.Dataset):
    def __init__(self, n: int, num_samples: int = 10000, seed: int | None = None):
        """
        Create a synthetic dataset where:
        - Label 0 corresponds to states when the first qubit is 1
        - Label 1 corresponds to states when the first qubit is 0
